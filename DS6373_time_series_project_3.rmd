...
### VAR Model
For our VAR model, we evaluated the full data set for aapl stock with near and far-term horizons. The optimal model for near-term predictions incorporated first-difference lagged data on all of the explanatory variables. When this model was evaluated for far-term prediction accuracy, we identified significant performance degradation. Un-lagged data was then used for far-term model creation and forecasts. The un-lagged data was observed performing better in the long-term and the lagged data was observed performing better in the short-term.  
...
plot(seq(1,360,1), aapl$Close[3883:4242], type = "l",xlim = c(0,365), ylab = "Close Price", main = "5 Day Forecast", xlab='Last Year in Days') + lines(seq(361,365,1), preds_5day$fcst$y1[,1], type = "l", col = "red") + lines(seq(361,365,1), aapl$Close[4243:4247], type = 'l', col='blue')
